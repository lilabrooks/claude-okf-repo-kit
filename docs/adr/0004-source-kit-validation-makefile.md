...
The Makefile runs syntax checks, optional ShellCheck linting, JSON validation, stale-reference and local-path scans, Markdown link checks, install simulations, idempotency checks, hook checks, and helper command smoke tests.

# Consequences

Maintainers have one command before publishing changes.

The Makefile is not part of the target repo installation contract.
