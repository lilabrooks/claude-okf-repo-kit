...
Amendment (2026-07-24, kit 0.3.12): the skill now renders a curated `templates/AGENTS.md` instead of directing the agent to hand-translate `CLAUDE.md`. The original decision judged the whole port to be judgment work; a live counter-example narrowed that. A hand-written `AGENTS.md` draft, produced by find-and-replace over the kit's own playbook, claimed Codex resolves `@` imports, named three kit files that do not exist, and wrote `.Codex` for `.codex` — every defect objective and mechanically checkable. So the *mechanics* of the port are template work (the four things that do not port are the same in every repo), while *scoping* the second agent and *adapting* the skills remain judgment and stay in the skill. The template is source-only and opt-in — the installers never write it, so the default install and the manifest do not grow, and the "kit is built for Claude Code" positioning is unchanged. `make parity` enforces the template against those three defect classes plus section pairing with `templates/CLAUDE.md`.
