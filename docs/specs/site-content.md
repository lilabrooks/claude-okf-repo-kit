...
- **Relative paths only** (ADR 0016): asset, navigation, and redirect targets are relative so the same files render at both the apex and the project base; `compact-theme.css` and its `./fonts/` references are relative. Absolute `canonical`, Open Graph, and schema.org URLs point at the project base (`https://lilabrooks.github.io/claude-okf-repo-kit/`), which is now the site's only home — the apex mirror ended when the experiment retired.
- **Not indexed** (2026-08-01): every page in `site/` carries `<meta name="robots" content="noindex, follow">`. The site stays publicly reachable as the archive of record but is withdrawn from search results. Keep the tag on any page added or restyled here.
...
- Publishing is one step: edit `site/` here and the project-base Pages redeploys. The apex-submodule bump described by ADR 0016 no longer applies — the `lilabrooks.github.io` repo dropped the `_kit` submodule on 2026-08-01 and now redirects to `lilabrooks.github.io/lila-agent-skills/`.
