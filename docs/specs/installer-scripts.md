...
- preflight the existing `docs/okf-map.yml` and every effective `layout:` path before its first mutation: the map must resolve to a regular file within the target; `specs_dir`, `adr_dir`, and `stamp_file` must be relative, contain no empty, `.` or `..` path segment or control character, and still resolve within the target after following any existing symlink. An unsafe value is a hard error. Rejection must create no backup directory, change no target file, and write nothing through a parent or symlink escape
...
The first-write preflight is an urgent source-only repair to this updater. It
does not make the installed `scripts/okf` helper safe for hostile map layouts,
and it must not be recorded as full closure of an umbrella finding that includes
that installed helper. Source-only publication follows ADR 0028.
