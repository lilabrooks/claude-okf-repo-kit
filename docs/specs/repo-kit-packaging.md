...
`README.md` must describe the optional second-agent usage: the goal loop stays with Claude Code; a second agent's committed config (such as `AGENTS.md` and `.codex/`) is tolerated by the shipped hooks as agent config; ported playbooks need explicit session-start reads instead of `@` imports and must state that the env-file read denial is Claude Code-only; hook mirrors in another agent's config are the owner's to keep in sync after kit upgrades, and because that sync is manual, a repo that maintains mirrors is advised to guard the invariant with a check in its own test or validation gate (mirrored hooks byte-identical, mirrored skill sets paired, skipped when the second stack is absent) rather than relying on discipline — the check belongs to the target repo, not the kit.
