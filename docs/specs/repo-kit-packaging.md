...
- carry the ADR review mechanics: the owner finds pending decisions by scanning for `status: proposed`; accepting flips the status and binds future work; rejecting reverts the work per the ADR's rollback trigger; the owner may direct Claude Code to make the status edit and any reversal
- when the goal is met, propose candidate next milestones sourced from `docs/log.md` known items, ADR revisit triggers, and extensions within the stated non-goals — flagging options that need a non-goal revision separately — and add nothing to `docs/GOAL.md` without the owner's confirmation
