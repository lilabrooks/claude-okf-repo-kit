...
The updater never overwrites your work. Kit-managed files (`scripts/okf`, the two hooks, and the six `okf-*` skills) are refreshed in place — after a backup under `.okf-kit-backups/<timestamp>/` — only when a digest manifest proves the current content is the kit's own unedited output; a script you edited is left exactly as you had it, with the new kit version written beside it as a numbered candidate (such as `check-docs-sync.2.sh`) under "Needs review". Changed templates get same-folder numbered candidates (such as `CLAUDE.2.md`) the same way — except for a playbook that is already finished: when your `CLAUDE.md`/`AGENTS.md` derives from the kit template and has no `[bracket]` blanks left, staging the whole bracketed template beside it would review to nothing, so the updater writes only the kit-only delta and says so under "Skipped". Across repeated upgrades it refreshes its own untouched candidates in place instead of stacking `CLAUDE.3.md`, `CLAUDE.4.md`, and so on — only candidates you edited keep their content and get a new number beside them. Review the candidates, merge what you want, and delete the rest — the summary's "Advisories" section helps here: it points at the kit-only template delta the updater writes beside the backups when the playbook template changed across releases, names any byte-identical hook mirror no `mirrors:` declaration covers, and lists `okf-*` skills a second agent's skill home is missing after a release adds one. The manifest lives in the git-ignored `.okf-kit-backups/`, so it never leaves your working copy; repos installed or updated before the manifest existed take the safe path (preserve plus candidate) once, then opt in. Repos installed before the version stamp existed stay silent about drift until one updater run (or a hand-added `kit_version` in `docs/index.md`) opts them in — `verify-install` warns when the stamp is missing.
...
- Declare the mirror directories in a top-level `mirrors:` list in `docs/okf-map.yml` (for example `- .codex/hooks`), and the safe updater keeps them current on every kit upgrade (ADR 0021): each declared mirror gets the same hooks through the same provenance path as `.claude/hooks/` — created when absent, refreshed in place when provably unedited kit output, preserved with a numbered candidate when you've edited it. A mirror you set up or adopted by hand counts as unedited too — matching the `.claude/hooks/` original is exactly what the declaration asks of it — so it is refreshed in place rather than re-staging the same no-op candidate on every upgrade. `verify-install` warns when a declared mirror is missing a hook or has drifted. Hooks only: mirrored skills carry per-agent substitutions the kit can't reproduce, so they stay yours to maintain, and an undeclared mirror keeps the manual re-sync behavior — the updater's own "Advisories" section, `verify-install`, and the SessionStart hook all flag byte-identical hook copies that no `mirrors:` entry covers, so an undeclared mirror doesn't stay invisible.
...
**Kit candidate review (session start).** The same hook also names any numbered kit candidates (`CLAUDE.2.md` and similar) still unresolved from an install or upgrade, using the machine-local candidate manifest, so review copies don't slip into a first commit. `verify-install` warns about the same files. Merge what you want into the live files, delete the candidates, and both stay silent; the check is local, offline, and inactive when the manifest is absent.
