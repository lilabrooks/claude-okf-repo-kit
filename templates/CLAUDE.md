...
- When every milestone is checked and the success criteria pass, report that the goal is met and stop building. Then offer candidate next milestones for me to choose from: known items in `docs/log.md`, revisit triggers in accepted ADRs, and extensions that fit the stated non-goals — with options that would first need me to revise a non-goal listed separately. Proposing is not adding: nothing enters `docs/GOAL.md` without my confirmation, and at my direction you draft the chosen milestones with their verifications.
