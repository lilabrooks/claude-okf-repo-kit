...
Push back on answers that can't be checked mechanically — "migrate the API to GraphQL" is a direction, not a done state; keep asking until each answer would let you verify progress yourself. When a verification step names a tool the repo itself doesn't provide — a local mail sink, a browser driver, a database client — confirm during the interview that it is installed, or write the step as owner-gated so the loop expects to pause there for my go-ahead instead of stalling on it mid-milestone. Draft the remaining milestones from my answers — ending, by default, with a README-quickstart milestone whose verification is reproducing the quickstart on a clean checkout; I can drop it — and confirm the finished `docs/GOAL.md` with me before starting the loop. In an existing codebase, propose answers from the code first — detected test commands, structure, conventions — and let me correct them rather than asking cold. Manual editing stays a valid alternative; never overwrite goal content I wrote by hand.
...
The same SessionStart hook also compares `kit_version` in `docs/index.md` — stamped by the kit installers — against the kit's published `VERSION` on the source kit's main branch. When it reports drift, tell me and recommend re-running `scripts/update-existing-repo` from an up-to-date clone of the kit. The updater is non-destructive: it refreshes kit-managed scripts in place only when they are provably unedited kit output (backing them up first), and otherwise writes same-folder numbered candidates (such as `CLAUDE.2.md` for changed templates, or `check-docs-sync.2.sh` beside a script this repo edited); reviewing and merging candidates is my decision. If `docs/index.md` carries no `kit_version`, the hook stays silent and this policy is inactive.
