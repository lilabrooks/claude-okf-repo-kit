...
<!-- Template. Fill every [bracket], update the timestamp, then delete this comment.
     Codex ignores the frontmatter; it exists to make this file a valid OKF
...
Codex does not resolve `@` imports. Read these three small files at the start of every session, before the first task — they carry the goal and the knowledge indexes. Keep them small; full specs and ADRs stay on disk until a task needs them.
...
- `docs/GOAL.md` is one of the session-start reads above, so the goal is in context from the first task. Re-read it during a session only after it changes.
...
If `/docs/specs` or `/docs/adr` doesn't exist yet, create this structure before the first task. The session-start reads above assume this structure exists; creating it makes them resolve from the next session on:
...
- `.codex/hooks.json` — project hook wiring for Codex: the same Stop and SessionStart guardrail hooks that `.claude/settings.json` wires for Claude Code. Committed. Codex loads it only when this project's `.codex` layer is trusted.
- `.codex/hooks/check-docs-sync.sh` — Stop hook, invoked via `bash` so no executable bit is needed. Committed, and byte-identical to `.claude/hooks/check-docs-sync.sh`. Don't move, rename, or disable it; if it blocks a stop, do the doc update it asks for.
- `.codex/hooks/check-okf-version.sh` — SessionStart hook, invoked via `bash`. Committed, byte-identical to its `.claude/hooks/` original. Reports OKF spec version drift; act per the OKF version policy above.
- `.agents/skills/okf-*/SKILL.md` — the `okf-*` workflow procedures adapted for this agent, when it loads skills at all. Committed. Codex support for skills varies: if they don't load, every rule in this file still binds on its own — the one-liners here are written to stand alone.
...
- Personal overrides live outside this repo — user-level Codex config and personal global instructions under `~/.codex/`. Never commit personal config into the repo.

This playbook is itself the guided second-agent port the kit's `okf-second-agent` skill describes (the `.claude/skills/` copy carries the full procedure). The binding rules it encodes: the session-start reads above replace Claude Code's `@` imports, which Codex does not resolve; guardrails that are mechanical only in Claude Code are stated honestly as policy here; the hook copies under `.codex/hooks/` stay byte-identical to `.claude/hooks/` and their directory is declared in a top-level `mirrors:` list in `docs/okf-map.yml`, so the safe updater keeps them current on kit upgrades; any adapted skills stay paired with the `.claude/skills/` set by hand; and a parity check belongs in this repo's own test gate.
...
Everything else agent-related is committed: `AGENTS.md`, `.codex/hooks.json`, `.codex/hooks/`, the Claude Code equivalents (`CLAUDE.md`, `.claude/`), `scripts/okf`, and all of `/docs`.
...
- Claude Code's shipped settings mechanically deny reading `.env` files; Codex has no equivalent deny rule wired here, so for this agent the same rule is policy, not a mechanical gate: never read or print `.env` contents. Never remove or weaken the Claude-side denial either; if a task seems to require reading `.env`, stop and ask me.
